...
var testPacketIPv6Routing = []byte{
	0x60, 0x00, 0x00, 0x00, 0x00, 0x38, 0x2b, 0x40, 0x20, 0x01, 0x0d, 0xb8, 0x00, 0x00, 0x00, 0x00,
	0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x20, 0x01, 0x0d, 0xb8, 0x00, 0x00, 0x00, 0x00,
	0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x02, 0x3b, 0x06, 0x04, 0x02, 0x03, 0x00, 0x00, 0x00,
...
func TestPacketIPv6RoutingSerialize(t *testing.T) {
	var serialize = make([]gopacket.SerializableLayer, 0, 2)
	var err error
	...
	srh.NextHeader = IPProtocolNoNextHeader
	srh.SourceRoutingIPs = append(srh.SourceRoutingIPs, net.ParseIP("2001:db8::2"))
	srh.SourceRoutingIPs = append(srh.SourceRoutingIPs, net.ParseIP("2001:db8::3"))
	srh.SourceRoutingIPs = append(srh.SourceRoutingIPs, net.ParseIP("2001:db8::4"))
	ip6.Routing = srh
	...
	got := buf.Bytes()
	want := testPacketIPv6Routing
	if !bytes.Equal(got, want) {
		t.Errorf("IPv6Routing serialize failed:\ngot:\n%#v\n\nwant:\n%#v\n\n", got, want)
	}
}

func TestPacketIPv6RoutingDecode(t *testing.T) {
	ip6 := &IPv6{
		BaseLayer: BaseLayer{
			Contents: []byte{
				0x60, 0x00, 0x00, 0x00, 0x00, 0x38, 0x2b, 0x40, 0x20, 0x01, 0x0d, 0xb8, 0x00, 0x00, 0x00, 0x00,
	...
	rt := &ip6.rt
	rt.ipv6ExtensionBase = ipv6ExtensionBase{
		BaseLayer: BaseLayer{
			Contents: []byte{
				0x3b, 0x06, 0x04, 0x02, 0x03, 0x00, 0x00, 0x00,
				0x20, 0x01, 0x0d, 0xb8, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x02,
				0x20, 0x01, 0x0d, 0xb8, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x03,
				0x20, 0x01, 0x0d, 0xb8, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x04,
			},
	...
	rt.SegmentsLeft = 2
	rt.LastEntry = 3
	rt.SourceRoutingIPs = append(rt.SourceRoutingIPs, net.IP{0x20, 0x01, 0x0d, 0xb8, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x02})
	rt.SourceRoutingIPs = append(rt.SourceRoutingIPs, net.IP{0x20, 0x01, 0x0d, 0xb8, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x03})
	rt.SourceRoutingIPs = append(rt.SourceRoutingIPs, net.IP{0x20, 0x01, 0x0d, 0xb8, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x04})
	ip6.Routing = rt

	p := gopacket.NewPacket(testPacketIPv6Routing, LinkTypeRaw, gopacket.Default)
	if p.ErrorLayer() != nil {
		t.Error("Failed to decode packet:", p.ErrorLayer().Error())
	}
	...
	if got, ok := p.Layer(LayerTypeIPv6).(*IPv6); ok {
		want := ip6
		want.Routing = got.Routing // avoid comparing pointers
		if !reflect.DeepEqual(got, want) {
			t.Errorf("IPv6 packet processing failed:\ngot:\n%#v\n\nwant:\n%#v\n\n", got, want)
		}
	} else {
	...
	if got, ok := p.Layer(LayerTypeIPv6Routing).(*IPv6Routing); ok {
		want := rt
		if !reflect.DeepEqual(got, want) {
			t.Errorf("IPv6Routing packet processing failed:\ngot\n%#v\n\nwant:\n%#v\n\n", got, want)
		}
	} else {
