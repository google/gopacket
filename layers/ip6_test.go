...
func TestPacketIPv6SegmentRoutingHeaderDecode(t *testing.T) {
	...
	if packet.ErrorLayer() != nil {
		t.Errorf("IPv6 packet SRH decoding failied: %v", packet.ErrorLayer())
	}
	if len(packet.Layers()) < 2 {
		t.Errorf("IPv6 packet SRH decoding failed, expecting at least 2 decoded layers")
	}
	if packet.Layers()[0].LayerType() != LayerTypeIPv6 {
		t.Errorf("Expecting IPv6 header as first layer")
	}
	if packet.Layers()[1].LayerType() != LayerTypeIPv6SegmentRouting {
		t.Errorf("Expecting IPv6 SRH header as second layer")
	}
	srh := packet.Layers()[1].(*IPv6SegmentRoutingHeader)
	if srh.SegmentsLeft != 1 {
		t.Errorf("Expecting SRH SL=1")
	}
	if len(srh.SourceRoutingIPs) != 1 {
		t.Errorf("Expecting 1 SID")
	}
