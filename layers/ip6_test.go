...
func TestPacketIPv6RoutingType0Decode(t *testing.T) {
	...
	if got, ok := p.Layer(LayerTypeIPv6Routing).(*IPv6RoutingType0); ok {
		...
		want.NextHeader = IPProtocolNoNextHeader
		want.HeaderLength = uint8(4)
		want.ActualLength = 40
		want.RoutingType = 0
		want.SegmentsLeft = 2
		want.Reserved = []byte{0x00, 0x00, 0x00, 0x00}
		want.SourceRoutingIPs = append(want.SourceRoutingIPs, net.IP{0x20, 0x01, 0x0d, 0xb8, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x03})
		want.SourceRoutingIPs = append(want.SourceRoutingIPs, net.IP{0x20, 0x01, 0x0d, 0xb8, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x04})
		if !reflect.DeepEqual(got, want) {
