...
// OpenLive opens a device and returns a *PcapHandle
func OpenLive(device string, snaplen int32, promisc bool, timeout_ms int32) (handle *PcapHandle, err error) {
	var buf *C.char
...
func OpenOffline(file string) (handle *PcapHandle, err error) {
	var buf *C.char
	buf = (*C.char)(C.calloc(ERRBUF_SIZE, 1))
	h := new(PcapHandle)
...
func (p *PcapHandle) NextEx() (pkt *Packet, result int32) {
...
func (p *PcapHandle) DataLink() LinkType {
	return LinkType(C.pcap_datalink(p.cptr))
}

func (p *PcapHandle) SetDatalink(dlt LinkType) error {
