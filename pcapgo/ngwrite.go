...
func (w *NgWriter) WriteCustomBlock(blockType uint32, body []byte) {
	bodyLength := len(body)
	padding := (4 - bodyLength&3) & 3
	totalLength := bodyLength + padding + 12

	binary.Write(w.w, binary.LittleEndian, blockType)
	binary.Write(w.w, binary.LittleEndian, uint32(totalLength))
	w.w.Write(body)
	w.w.Write(make([]byte, padding))
	binary.Write(w.w, binary.LittleEndian, uint32(totalLength))
}
...
func (w *NgWriter) WriteTLSKeyLog(kl []byte) {
	klLength := len(kl)
	padding := (4 - klLength&3) & 3
	totalLength := klLength + padding + 20

	binary.Write(w.w, binary.LittleEndian, uint32(ngBlockTypeDecryptionSecrets))
	binary.Write(w.w, binary.LittleEndian, uint32(totalLength))
	binary.Write(w.w, binary.LittleEndian, uint32(0x544c534b)) // NSS key log format
	binary.Write(w.w, binary.LittleEndian, uint32(klLength))
	w.w.Write(kl)
	w.w.Write(make([]byte, padding))
	binary.Write(w.w, binary.LittleEndian, uint32(totalLength))
}
